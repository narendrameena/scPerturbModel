...
Outputs: results/tables/edistance_dev.csv, results/figures/edistance_dose_moa.png
"""
...
from perturbmodel.evaluation import e_distance  # noqa: E402
from perturbmodel.utils import save_figure  # noqa: E402
...
for line in lines:
    ...
    dense_sum = np.zeros(n_genes)
    dense_sq = np.zeros(n_genes)
    X = np.zeros((len(cells), 0), dtype=np.float32)  # placeholder
    # first compute log1p CPM per cell into a temp dense matrix in chunks
    ...
    print(f"{line}: {sum(1 for r in rows if r[0]==line)} conditions", flush=True)
    del M, Mh
...
# aggregate: median across plates then lines
...
axes[0].set_xticks(range(len(doses)), [f"{d} uM" for d in doses])
axes[0].set_ylabel("log1p E-distance (median over lines)")
axes[0].set_title("A  E-distance rises with dose", loc="left", fontweight="bold")

top = (agg[agg.conc == max(doses)].groupby("moa-fine", observed=True)
       .med_e.median().sort_values(ascending=False).head(12))
axes[1].barh(range(len(top))[::-1], np.log1p(top.values), height=0.62, color=BLUE)
