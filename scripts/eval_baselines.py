...
# ---------------- split ----------------
test_mask = RNG.random(len(G)) < TEST_FRAC
train_pairs = set(zip(G.drug[~test_mask], G.conc[~test_mask]))
usable = np.array([(d, c) in train_pairs for d, c in zip(G.drug, G.conc)])
test_mask &= usable
print(f"train {np.sum(~test_mask)}, test {test_mask.sum()} "
      f"({(~usable).sum()} triples dropped: (drug,conc) fully in test)")
...
for ax, metric, title in ((axes[0], "r_hvg", "A  r on 2,000 most responsive genes"),
    ...
    data = [res[res.baseline == b][metric].dropna() for b in ("no_change", "additive_shift")]
    ...
    ax.set_xticks([0, 1], ["no change", "additive shift"])
