...
def main():
    ...
    print(f"\norgan concordance (lines with reference tissue, n={len(with_ref)}):"
          f" top1 {with_ref.match_top1.mean():.1%}, "
          f"top3 {with_ref.match_top3.mean():.1%}")
    print("\ndiscordant lines (top3 miss):")
    print(with_ref[~with_ref.match_top3.astype(bool)]
    ...
    axes[1].bar([0, 1], rates, width=0.6, color="#2a78d6")
    for x, v in enumerate(rates):
        axes[1].text(x, v + 0.02, f"{v:.0%}", ha="center", fontsize=10)
    axes[1].set_xticks([0, 1], ["top-1", "top-3"])
    ...
    d = save_figure(fig, "cell_type_audit", FIG,
                    source_data={"audit": res,
                                 "heatmap": pd.DataFrame(
                                     heat[order_lines],
                                     index=[res.cell_name[i] for i in order_lines],
                                     columns=tissues)},
                    script=__file__)
