...
def _ks(a: np.ndarray, b: np.ndarray) -> float:
    """Two-sample Kolmogorov-Smirnov statistic (no p-value needed)."""
    a = np.sort(a)
    b = np.sort(b)
...
def main():
    ...
    for (ln, dr, cc, pl), grp in treated.groupby(
        ...
        for comp in comps:
            ...
            # Normalise by the CONTROL SPREAD, not by the mean shift: dividing
            # by a near-zero mean shift made weak responses look compositional
            # regardless of their shape (the first version of this metric did
            # exactly that).
            spread = float(np.subtract(*np.quantile(k, [0.75, 0.25])) + 1e-9)
            slope = float(np.polyfit(QS, shift_q, 1)[0])
            uniformity = float(1.0 - min(1.0, abs(slope) / spread))
            # Principled test. Under a PURE LOCATION SHIFT the treated cells
            # are distributed exactly like the controls moved by mean_shift, so
            # KS(treated, control + mean_shift) should equal the sampling-noise
            # floor measured by splitting the controls in half. Excess over that
            # floor is evidence of a SHAPE change, i.e. a subpopulation effect.
            ks_shift = _ks(t, k + mean_shift)
            h = len(k) // 2
            ks_floor = max(_ks(k[:h], k[h:]), 1e-6)
            shape_excess = float(ks_shift / ks_floor)
            ...
            recs.append({
                ...
                "uniformity": uniformity, "ks_shift": float(ks_shift),
                "ks_floor": float(ks_floor), "shape_excess": shape_excess,
    ...
    frac_comp = float((res.shape_excess > 2).mean())
    print(f"\nshape_excess (KS vs shifted control / within-control floor): "
          f"median {res.shape_excess.median():.2f}; "
          f">2 in {frac_comp:.1%} of comparisons")
    print("  1 = pure location shift (all cells move together); "
          ">>1 = shape change")
    strong = res[res.mean_shift.abs() > res.mean_shift.abs().quantile(0.9)]
    print(f"  strongest-decile responses: median shape_excess "
          f"{strong.shape_excess.median():.2f}, "
          f"var_ratio {strong.var_ratio.median():.2f}")
    ...
    axes[0].hist(np.log2(res.shape_excess.clip(0.25, 16)), bins=60,
                 color=BLUE, alpha=0.8)
    axes[0].axvline(0, color="#333333", ls="--", lw=1.2)
    axes[0].set_xlabel("log2 shape excess  (0 = pure location shift)")
    axes[0].set_ylabel("condition × component")
    axes[0].set_title("A  Uniform shift or subpopulation?", loc="left",
                      fontweight="bold", fontsize=10)
    axes[0].text(0.03, 0.95,
                 f"median {res.shape_excess.median():.2f}×\n"
                 f"{frac_comp:.0%} above 2×",
