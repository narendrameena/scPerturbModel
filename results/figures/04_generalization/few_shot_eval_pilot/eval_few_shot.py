...
def main():
    ...
    for fi, ln in enumerate(fold_lines):
        ...
        for seed in range(args.seeds):
            ...
            for strategy in ("random", "high_effect"):
                for k in K_VALUES:
                    if strategy == "random":
                    ...
                    for j, i in enumerate(eval_idx):
                        ...
                        recs.append({"line": ln, "seed": seed, "k": k,
                                     "strategy": strategy, "drug": G.drug[i],
                                     "conc": G.conc[i],
    ...
    summ = (res.groupby(["strategy", "k"], dropna=False).r_de100
            .agg(["mean", "median", "size"]).round(4))
...
def make_figure(res: pd.DataFrame, suf: str):
    ...
    # A: k-curve, median +- IQR per strategy
    for strat in ("random", "high_effect"):
        sub = res[res.strategy == strat]
        if not len(sub):
            continue
        g = sub.groupby("k").r_de100
        ks = sorted(sub.k.dropna().unique())
        med = [g.median()[k] for k in ks]
        lo = [np.percentile(sub[sub.k == k].r_de100.dropna(), 25) for k in ks]
        hi = [np.percentile(sub[sub.k == k].r_de100.dropna(), 75) for k in ks]
    ...
    axes[0].set_title("A  Few-shot transfer curve", loc="left",
