...
def main():
    ...
    recs, state_recs, scores_by_unit = [], [], {}
    ...
    # ---------------- interaction-only rescan ----------------
    # The raw scan conflates "this genotype responds more overall" with "this
    # genotype responds specifically to THIS mechanism": a driver that marks
    # generally responsive lines tops every class. Z-scoring each line ACROSS
    # mechanisms removes its general responsiveness, leaving only the
    # genotype x mechanism interaction.
    S = pd.DataFrame(scores_by_unit)                   # lines x units
    S = S.sub(S.mean(axis=1), axis=0).div(S.std(axis=1) + 1e-9, axis=0)
    inter = []
    for unit in S.columns:
        col = S[unit].dropna()
        for g in driver_genes:
            gm = mut.loc[col.index, g].to_numpy().astype(bool)
            if gm.sum() < MIN_MUT or (~gm).sum() < MIN_WT:
                continue
            _, p = stats.mannwhitneyu(col[gm], col[~gm], alternative="two-sided")
            inter.append({"unit": unit, "driver_gene": g,
                          "n_mut": int(gm.sum()), "n_wt": int((~gm).sum()),
                          "effect": float(np.median(col[gm])
                                          - np.median(col[~gm])),
                          "p": float(p)})
    inter = pd.DataFrame(inter)
    if len(inter):
        inter["q"] = bh_fdr(inter.p.to_numpy())
        inter = inter.sort_values("p")
        inter.to_csv(TAB / f"genotype_interaction_only_{args.tag}.csv",
                     index=False)
        print(f"\n=== interaction-only scan (line responsiveness removed) ===")
        print(f"{len(inter)} tests; {(inter.q < 0.1).sum()} at FDR<0.10")
        print(inter.head(15).to_string(index=False))
